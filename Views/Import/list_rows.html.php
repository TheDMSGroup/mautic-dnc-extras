...
/** @var \MauticPlugin\DoNotContactExtrasBundle\Entity\DncImport $item */
?>
<?php foreach ($items as $item): ?>
    <tr>
        <td class="col-actions text-center">
            <span class="label label-<?php echo $item->getSatusLabelClass(); ?>">
                <?php echo $view['translator']->trans('mautic.dnc.import.status.'.$item->getStatus()); ?>
            </span>
        </td>
        <td>
            <div>
                ...
                <?php if ($view['security']->hasEntityAccess(true, $permissions[$permissionBase.':viewother'], $item->getCreatedBy())) : ?>
                    <a href="<?php echo $view['router']->path(
                        $actionRoute,
                        ['objectAction' => 'view', 'objectId' => $item->getId(), 'object' => 'dnc']
                    ); ?>" data-toggle="ajax">
