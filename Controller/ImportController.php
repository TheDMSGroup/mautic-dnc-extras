...
class ImportController extends FormController
{
    ...
    {
        $this->get('session')->set('mautic.import.dnc', 'dnc');
    ...
    {
        $object = $this->get('session')->get('mautic.import.dnc');
    ...
    {
        return $this->viewStandard($objectId, 'dncimport', 'donotcontactextras');
    }
    ...
    public function cancelAction($objectId)
    {
        ...
        $import      = $importModel->getEntity($session->get('mautic.dnc.import.id', null));
    ...
    public function newAction($objectId = 0, $ignorePost = false)
    {
        ...
        $session = $this->get('session');
        $session->set('mautic.import.dnc', $object);
        ...
        $action   = $this->generateUrl('mautic_dnc.import_action', ['object' => $object, 'objectAction' => 'new']);
        ...
        switch ($step) {
            case self::STEP_UPLOAD_CSV:
                ...
                $form = $this->get('form.factory')->create('dnc_import', [], ['action' => $action]);
                break;
            case self::STEP_MATCH_FIELDS:
                $dncFields     = ['name' => 'Value'];
                ...
                switch ($step) {
                    ...
                    case self::STEP_MATCH_FIELDS:
                        ...
                        if (empty($matchedFields)) {
                            $form->addError(
                                new FormError(
                                    $this->get('translator')->trans('mautic.dnc.import.matchfields', [], 'validators')
                                )
                            );
                        } else {
                            $defaultOwner = $this->get('security.token_storage')->getToken()->getUser()->getId();

                            /** @var \MauticPlugin\MauticDoNotContactExtrasBundle\Entity\DncImport $import */
                            $import = $importModel->getEntity();
        ...
        if (self::STEP_UPLOAD_CSV === $step || self::STEP_MATCH_FIELDS === $step) {
            $contentTemplate = 'MauticDoNotContactExtrasBundle:Import:new.html.php';
            $viewParameters  = ['form' => $form->createView()];
        } else {
            $contentTemplate = 'MauticDoNotContactExtrasBundle:Import:progress.html.php';
            $viewParameters  = [
                'progress'   => $progress,
                'import'     => $import,
                'complete'   => $complete,
    ...
    {
        $progress = $this->get('session')->get('mautic.dnc.import.progress', [0, 0]);
    ...
    {
        /** @var \MauticPlugin\MauticDoNotContactExtrasBundle\Model\DncImportModel $importModel */
        $importModel = $this->getModel('donotcontactextras.dncimport');
    ...
    public function getViewArguments(array $args, $action)
    {
        switch ($action) {
            case 'view':
                ...
                $args['contentTemplate'] = 'MauticDoNotContactExtrasBundle:Import:details.html.php';
    ...
    public function generateUrl($route, $parameters = [], $referenceType = UrlGeneratorInterface::ABSOLUTE_PATH)
    {
        if (!isset($parameters['object'])) {
            $parameters['object'] = $this->request->get('object', 'dnc');
        }
    ...
    {
        return 'donotcontactextras.dncimport';
    }
    ...
    {
        return 'dnc.import'.(($objectId) ? '.'.$objectId : '');
    }
    ...
    {
        return 'MauticDoNotContactExtrasBundle:Import';
    }
    ...
    {
        return 'mautic_dnc.import_index';
    }
}
